...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] transition duration-200 hover:-translate-y-1 hover:border-[#ee4d2d]/50 hover:bg-white/[0.06] hover:shadow-xl hover:shadow-black/40"
    >
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        <h3 className="line-clamp-2 text-sm font-medium text-slate-100 group-hover:text-[#ff8a3d]">
          {product.name}
        </h3>
        <p className="text-xs text-slate-500">{product.shop}</p>

        <div className="mt-auto flex items-baseline gap-2 pt-1">
          <span className="text-lg font-bold text-[#ff8a3d]">{vnd(product.currentPrice)}</span>
